...
- **Visual design system (Typeless-inspired, v0.18.0)**: the UI uses a warm-neutral **`stone`** palette as its base (not `gray`) with **black as the primary color** — primary buttons, active states, and emphasis are `stone-900` in light mode and invert to `stone-100`/white in dark mode; blue is no longer used as an accent. Reusable component classes are defined in `src/renderer/index.css` under `@layer components` and should be preferred over re-deriving inline class strings: `.btn-primary` (black pill button, inverts in dark), `.btn-ghost` (secondary pill), `.icon-btn` (circular icon button), `.input-field` (rounded `stone` input/textarea), `.card` (`rounded-2xl` filled surface), `.segmented` / `.segmented-item` / `.segmented-item-active` (pill segmented control), `.page-title` (`text-2xl` bold page heading). Radius convention: buttons/segmented = `rounded-full`, inputs = `rounded-xl`, cards/dialogs = `rounded-2xl`, list items = `rounded-xl`. Panels follow a Typeless layout: a large bold page title (full-width sidebar views) or `text-xl` heading (drawer record views) with a count pill and a right-aligned black "+ 新增" primary action. Semantic colors are kept only where they carry meaning: red for danger/cancel, emerald for the recording "complete" button, amber for the favorite star and the comparison-mode warning. When adding UI, keep to these primitives so light/dark stay consistent.
...
- Sidebar header: `Sidebar` has a fixed width (`w-56`) and always shows the app logo (`src/renderer/assets/logo.png`) next to the "翻译助手" title. There is no collapse/expand toggle.
